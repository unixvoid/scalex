...
// Optimized settling detection using EMA and rate-of-change
// Returns smoothed weight value when settled, negative if still settling
...
// get current weight threshold value
float HX711_get_weight_threshold();

// get current stability time in ms
int HX711_get_stability_time_ms();

// update config values from NVS
...
// places calibration weight on scale and calculates scale factor
